...
import numpy as np
import random
from aliasing_test import down_sampling
...
def get_minibatch(batch, train = True, test = False):
	data = []
	ground_truth = []
	for ind in batch:
		if train:
...
def cost_superRes(Z, Y):
	cost_superRes = tf.reduce_mean(tf.pow(Z - Y,2), axis = (-1,-2,-3))
	return cost_superRes
	...
	with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
		...
		for epoch in range(num_epochs):
			...
			for cpt in range(num_minibatches):
				minibatch_X, minibatch_Y = get_minibatch(mini_batches[cpt])
				...
				print(model_type + " - epoch : %i/%i, batch : %i/%i, current training error : %f (res) - %.2f (bounds)"
			...
			val_cost = 0.
			for cpt in range(num_minibatches):
				minibatch_X,minibatch_Y = get_minibatch(mini_batches[cpt], train = False)
			...
			if epoch > 0 and val_cost < best_val:
				...
				saver.save(sess,  "../checkpoint_model/superRes_" + model_type + ".ckpt")
	plt.plot(costs)
	plt.xlabel('steps')
	plt.ylabel('error')
	plt.title('cout en train ' + model_type)
	plt.savefig('cout en train ' + model_type + '.png', bbox_inches = 'tight')
	plt.close()
	plt.plot(val_costs)
	plt.xlabel('steps')
	plt.ylabel('error')
	plt.title('cout en val ' + model_type)
	plt.savefig('cout en val ' + model_type + '.png', bbox_inches = 'tight')
	plt.close()
def reload(model_type):
	...
	saver.restore(sess, "../checkpoint_model/superRes_" + model_type + ".ckpt")
